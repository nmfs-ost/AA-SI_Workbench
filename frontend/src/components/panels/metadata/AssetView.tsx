...
import type { AssetMetadata } from '../../../state/activeSubject';

/**
 * The NCEI view of the Metadata panel — catalogue metadata for an archive file.
 *
 * Lifted out of `MetadataPanel` unchanged when the panel gained a second view.
 * The two are not variants of one form: this reads a catalogue record that
 * already exists in memory and renders instantly, while the store view shells
 * out to `aa-store` and has loading and failure states. Sharing a component
 * between them would mean every field carrying a "present for one source only"
 * branch, which is how a metadata pane becomes unreadable.
 */
...
export function AssetView({ asset }: { asset: AssetMetadata }) {
  const theme = useTheme();
