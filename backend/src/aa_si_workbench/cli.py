...
def _npm() -> str:
    ...
    if npm is None:
        _fail("`npm` not found. Install Node.js 18+ (https://nodejs.org) to build the UI.")
...
def cmd_serve(args: argparse.Namespace) -> None:
    ...
    if dist is None:
        if args.no_build:
            _fail("UI not built and --no-build was set. Run `aa-workbench build` first.")
...
def cmd_dev(args: argparse.Namespace) -> None:
    ...
    api_env = {**os.environ, "AASI_NCEI_SOURCE": args.source}
    web_env = {
        **os.environ,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_serve.add_argument("--open", dest="open_browser", action="store_true", help="Open a browser.")
    p_serve.add_argument("--no-build", action="store_true", help="Fail instead of building the UI.")
    p_serve.set_defaults(func=cmd_serve)

    p_dev = sub.add_parser("dev", help="Run frontend + backend with hot reload (developers).")
    p_dev.add_argument("--web-port", type=int, default=5173, help="Vite dev port (default 5173).")
    p_dev.add_argument("--api-port", type=int, default=8000, help="API port (default 8000).")
    p_dev.add_argument(
        "--source", choices=["s3", "cache"], default="s3",
