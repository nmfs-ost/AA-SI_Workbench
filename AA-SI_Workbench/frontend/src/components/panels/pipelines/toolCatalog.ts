...
export const toolCatalog: readonly ToolTemplate[] = [
  ...
  /*
   * NOTE: `aa-seabed` is the expected name for the seabed-detection tool but it
   * was NOT verified against an installed AA-SI environment, unlike every other
   * entry here. Confirm with `aa-help --help` (or `ls $VIRTUAL_ENV/bin/aa-*`)
   * and correct the tool string and flags below if they differ. Nothing else
   * changes: the command preview, the Configuration panel and the
   * seabed-detection pipeline are all generated from this entry.
   */
  {
    tool: 'aa-seabed',
    label: 'Detect seabed',
