...
2. **API service (`backend/src/aa_si_workbench/api/`)** — a FastAPI application
   ...
   result retrieval) to the frontend over HTTP. It also exposes the *environment*
   the Workbench orchestrates (`/api/env`): which `aa-*` tools are installed, and
   an allow-listed `aa-setup` run to update them — see
