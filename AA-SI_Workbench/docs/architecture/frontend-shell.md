...
AppToolbar     (fixed height)   Open Save | Refresh | Run Stop | Settings … Update Feedback
DockLayout     (grows)          the Dockview docking surface
StatusBar      (fixed height)   slim status strip + environment-update indicator
DialogHost     (no height)      renders whichever shell dialog is open
```
...
render logic. Both dispatch the same `ShellActionId` union: window management
(`reset-layout`, `close-all-panels`, `open-panel`), `open-dialog`, and
`open-external`. Items with no action are inert placeholders with shortcut
hints, ready to connect as features land.
...
### 4. Dialog registry (shell modals)

`src/components/dialogs/registry.tsx` is the panel registry's counterpart for
modals that belong to the chrome rather than to a dock panel — About, the
environment updater, feedback. A definition is `{ id, component }`; menu items
and toolbar buttons reference the `id` (`action: 'open-dialog'`), the open state
lives in `state/dialogs.ts`, and `DialogHost` (mounted once in `AppShell`)
renders the result. No chrome component imports a dialog component.

The dialogs' own state lives in module stores, not in the components: an
`aa-setup` run takes minutes, so `state/environment.ts` owns the job and its
polling loop. Closing the dialog does not stop the update, the status bar shows
it, and reopening reattaches.
...
└── components/
    ...
    ├── dialogs/             # registry + DialogHost + one component per shell modal
...
## Adding a shell dialog

1. Write the component with the `DialogComponentProps` signature
   (`{ open, onClose, payload? }`).
2. Add its id to `DialogId` in `types/dialogs.ts` and one entry to
   `dialogDefinitions` in `components/dialogs/registry.tsx`.
3. Point a menu item or toolbar button at it:
   `{ action: 'open-dialog', dialogId: 'myDialog' }`.
...
- **Bundle size**: the production bundle is ~1,073 kB (~296 kB gzip): React + MUI +
  Dockview, plus ~300 kB of xterm.js for the terminal. Lazy-loading the terminal panel
  from the registry is the obvious lever if that matters. Fine for a desktop-class app; route- or panel-level
